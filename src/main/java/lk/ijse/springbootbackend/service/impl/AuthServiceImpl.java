...
public class AuthServiceImpl implements AuthService {
    ...
    public String register(RegisterDTO registerDTO) {
        ...
        // Create initial profile record based on Role
        if (savedAuth.getRole() == Role.CANDIDATE) {
            Candidate candidate = new Candidate();
            candidate.setAuth(savedAuth);
            candidate.setBio("Professional Candidate at ColloQ");
            ...
            interviewer.setStatus("PENDING"); // Pending until profile completion
    ...
    public AuthMeDTO getCurrentUser(String username) {
        ...
        if (Role.CANDIDATE.equals(auth.getRole())) {
            ...
            interviewerRepo.findByAuth(auth).ifPresent(i -> {
                // Use Interviewer status (PENDING/ACTIVE) instead of Auth status
                dto.setStatus(i.getStatus());
                dto.setBio(i.getBio());
                dto.setSpecialization(i.getSpecialization());
                dto.setCompany(i.getCompany());
                ...
                // Map Level details for Interviewer
                if (i.getLevel() != null) {
                    dto.setLevelName(i.getLevel().getName());
                    dto.setPrice(i.getLevel().getPrice());
                }
            });
        }
        return dto;
